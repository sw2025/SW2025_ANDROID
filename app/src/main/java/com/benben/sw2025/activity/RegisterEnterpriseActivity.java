...
public class RegisterEnterpriseActivity extends BenBenActivity implements View.OnClickListener {
    ...
    public void initData() {
        text_common_title.setText("企业认证");
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.text_submit:
                if (isCanSubmit()){
                    ToastUtils.shortToast(RegisterEnterpriseActivity.this , "可以提交");
                }
                break;
            default:
                break;
    ...
    private boolean isCanSubmit() {
        if (TextUtils.isEmpty(x_edit_enterprise_name.getText().toString().trim())){
            ToastUtils.shortToast(RegisterEnterpriseActivity.this , "请输入企业全称");
            return false ;
        }
        if (TextUtils.isEmpty(text02.getText().toString().trim())){
            ToastUtils.shortToast(RegisterEnterpriseActivity.this , "请选择企业规模");
            return false ;
        }
        if (TextUtils.isEmpty(text03.getText().toString().trim())){
            ToastUtils.shortToast(RegisterEnterpriseActivity.this , "请选择所在行业");
            return false ;
        }
        if (TextUtils.isEmpty(text04.getText().toString().trim())){
            ToastUtils.shortToast(RegisterEnterpriseActivity.this , "请选择企业所在地区");
            return false ;
        }
        if (TextUtils.isEmpty(x_edit_enterprise_des.getText().toString().trim())){
            ToastUtils.shortToast(RegisterEnterpriseActivity.this , "请输入企业简介");
            return false ;
        }
        if (TextUtils.isEmpty(path01)){
            ToastUtils.shortToast(RegisterEnterpriseActivity.this , "请上传营业执照");
            return false ;
        }
        if (TextUtils.isEmpty(path02)){
            ToastUtils.shortToast(RegisterEnterpriseActivity.this , "请上传宣传照片");
            return false ;
        }
        return true;
    }
    ...
    private void selectPic(int a) {
        ...
        ImageConfig imageConfig = new ImageConfig.Builder(
                ...
                .requestCode(a)
                .build();
        ImageSelector.open(RegisterEnterpriseActivity.this, imageConfig);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1 && resultCode == RESULT_OK && data != null) {
            ...
            Glide.with(RegisterEnterpriseActivity.this)
                    .load(pathList.get(0))
                    .into(image_show06);
    ...
    private void popUpWindow(final RelativeLayout relative) {
        ...
        switch (relative.getId()) {
            case R.id.relative_02:
                picker.addData("不限");
                picker.addData("20人以下");
                picker.addData("20-99人");
                picker.addData("100-499人");
                picker.addData("500-999人");
                picker.addData("1000-9999人");
                picker.addData("10000人以上");
                break;
            case R.id.relative_03:
                picker.addData("不限");
